...
import 'package:flutter_doctor_web_app/modals/DoctorSlotDetails.dart';
import 'package:flutter_doctor_web_app/modals/SpecialityClass.dart';
import 'package:flutter_doctor_web_app/modals/Test.dart';
...
class _StepThreeDetailsScreenState extends State<StepThreeDetailsScreen> {
  ...
  getSpecialities() async{
    ...
    if(response.statusCode == 200){
      ...
      for(int i=0;i<specialityClass.data.specialityData.length;i++){
        if(specialityClass.data.specialityData[i].name == widget.myData.departmentName){
          setState(() {
            departmentId = i+1;
          });
          break;
  ...
  fetchDoctorSlots() async{
    final response = await get("$SERVER_ADDRESS/api/getdoctorschedule?doctor_id=${widget.doctorId}");
    if(response.statusCode == 200){
      final jsonResponse = jsonDecode(response.body);
              ...
              child: DropdownButton(
                ...
                items: slotsInterval.map((x){
                  return DropdownMenuItem(
                    child: Text(x,style: TextStyle(fontSize: 14,color: Colors.black),),
                    value: x,
                  );
                }).toList(),
